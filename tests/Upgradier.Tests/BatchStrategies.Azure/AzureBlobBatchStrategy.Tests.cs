...
public class AzureBlobBatchStrategy_Tests : IClassFixture<AzuriteFixture>, IClassFixture<SqlServerDatabaseFixture>
{
    ...
    private async Task<BlobContainerClient> CreateBlobContainerClientIfNotExists()
    {
    ...
    [InlineData(false)]
    [InlineData(true)]
    public async Task Update_Using_Azurite_Updates_SqlServer_Database(bool useCache)
    {
        Source source = new ("test-source", SqlEngine.NAME, _sqlServerConnectionString);
        IEnumerable<Source> sources = [source];
        BlobContainerClient blobContainerClient = await CreateBlobContainerClientIfNotExists();
