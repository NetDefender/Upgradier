...
public sealed class FileBatchStrategy_Tests
{
    ...
    public async Task GetAllBatches_Works_When_Base_Directory_Not_Ends_With_DirectorySeparator()
    {
        ...
        FileBatchStrategy strategy = new($"Core{Path.DirectorySeparatorChar}Batches", new LogAdapter(null), null);
    ...
    {
        using CancellationTokenSource cancellationTokenSource = new();
        FileBatchStrategy strategy = new($"Core{Path.DirectorySeparatorChar}Batches{Path.DirectorySeparatorChar}", new LogAdapter(null), null);
        IEnumerable<Batch> batches = await strategy.GetBatchesAsync(cancellationTokenSource.Token);
        Assert.NotNull(batches.FirstOrDefault(s => s.VersionId == 1));
        Assert.NotNull(batches.FirstOrDefault(s => s.VersionId == 2));
        Assert.Equal(2, batches.Count());
    ...
    {
        using CancellationTokenSource cancellationTokenSource = new();
        FileBatchStrategy strategy = new($"Core{Path.DirectorySeparatorChar}Batches", new LogAdapter(null), "Dev");
        IEnumerable<Batch> batches = await strategy.GetBatchesAsync(cancellationTokenSource.Token);
        Assert.NotNull(batches.FirstOrDefault(s => s.VersionId == 1));
        Assert.NotNull(batches.FirstOrDefault(s => s.VersionId == 2));
    ...
    public void Ctor_Throws_If_BaseDirectory_Is_Null_Or_Empty()
    {
        ...
        {
            new FileBatchStrategy(null, new LogAdapter(null), null);
        });
        Assert.Throws<ArgumentException>("baseDirectory", () =>
        {
            new FileBatchStrategy(string.Empty, new LogAdapter(null), null);
        });
    }
    ...
    public async Task Ctor_Throws_If_File_Index_Json_Not_Exists()
    {
        ...
        {
            FileBatchStrategy strategy = new($"NonExistentDirectory{Path.DirectorySeparatorChar}Batches", new LogAdapter(null), null);
